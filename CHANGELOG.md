...
- Docker multi-container setups now mount `HERMES_WORKSPACE` at `/workspace` in the gateway container as well as the WebUI container, set the gateway/WebUI default workspace env vars to the in-container path, and start the gateway in `/workspace` so Telegram/CLI/cron sessions resolve relative paths against the mounted workspace.
